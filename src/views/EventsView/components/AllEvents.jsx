...
const AllEvents = () => {
    const [allowedDates, setAllowedDates] = useState(getDateRange(new Date(), 10));
    ...
    const filterEvents = (_events, _category, _date) => {
        const _mDate = moment(_date, dateFormat).startOf('day').toISOString();
        return _events.filter(event => {
            const isInDate = moment(event.string.date, dateFormat).toISOString() == _mDate;
            return isInDate && event.category.map(x => x.toLowerCase()).includes(_category.toLowerCase());
        });
    };
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                setEvents(_events);
                const _categories = _events.map(ev => ev.category.map(cat => cat.toLowerCase())).flat().filter(onlyUnique);
...
const EventList = ({ events, onItemClick }) => (
    <ul className="event-list">
        {events.map((event, idx) => <EventItem key={idx} {...event} onClick={onItemClick} />)}
    </ul>
)
const EventItem = ({ _id, name, onClick, location, string, content, counter }) => {
    const _onClick = () => {
        onClick(_id);
    }
    return (
        <li onClick={_onClick} className={classNames("event-item", { "full": counter === 0 })}>
            <div className="event-item-top">
                <h3 className="event-item-name">{name}</h3>
                <span className="event-item-details">{location}, {string.time}</span>
            </div>
